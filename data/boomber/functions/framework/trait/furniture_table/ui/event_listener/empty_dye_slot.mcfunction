...
execute unless data block ~ ~ ~ Items[{Slot: 17b}].Count run scoreboard players add #bb.fw.should_draw bb.success 1
...
execute if data block ~ ~ ~ Items[{Slot: 17b}].tag{ucit: {id: "clay_slot", from: "boomber:framework"}} unless block ~ ~ ~ minecraft:barrel{Items: [{Slot: 17b, Count: 1b}]} run scoreboard players add #bb.fw.should_draw bb.success 1